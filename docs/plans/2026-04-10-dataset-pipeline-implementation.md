# Dataset Optimization Pipeline Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Port SelVA's chainable audio dataset preprocessing pipeline to Foley, adapted for DAC's 48kHz neural codec, with all nodes renamed to avoid conflicts.

**Architecture:** 12 new ComfyUI nodes in `nodes_dataset.py` passing a custom `FOLEY_AUDIO_DATASET` type through a chainable pipeline: Loader → Resampler → LUFS Normalizer → Compressor → HF Smoother → Augmenter → Inspector → Saver. Plus 3 single-audio pre/post-processor nodes. SpectralMatcher uses reference-based approach (no hardcoded stats) since DAC's latent distribution differs from BigVGAN mel-spectrogram.

**Tech Stack:** PyTorch, torchaudio, soxr (resampling), pyloudnorm (LUFS), pedalboard (compression/filtering), audiomentations (optional augmentation), soundfile (I/O).

**Key differences from SelVA:**
- All nodes prefixed `Foley` instead of `Selva` to avoid ComfyUI conflicts
- Default sample rate: 48000 Hz (vs 44100/16000)
- HF Smoother default cutoff: 16000 Hz (vs 12000) — DAC handles HF better than BigVGAN
- SpectralMatcher: Reference-based (user provides reference dir) instead of hardcoded VAE stats
- HarmonicExciter: Ported but less critical — DAC single-stage codec has better HF fidelity than BigVGAN vocoder
- Category: `audio/HunyuanFoley/Dataset` and `audio/HunyuanFoley/Audio`

**Critical notes:**
- The `FOLEY_AUDIO_DATASET` type is a Python list of dicts: `{"waveform": [1, C, L], "sample_rate": int, "name": str}`
- All dataset nodes accept and return this type for chaining
- `soxr`, `pyloudnorm`, `pedalboard`, and `soundfile` are optional dependencies — nodes that need them import at call time and give clear error messages if missing
- The pipeline is independent of the LoRA training plan and can be executed in parallel

---

### Task 1: Create `nodes_dataset.py` — Loader, Resampler, LUFS Normalizer

**Files:**
- Create: `nodes_dataset.py`

**Step 1: Write the file with core utilities and first 3 nodes**
...
git commit -m "feat: add Foley dataset pipeline — Loader, Resampler, LUFS Normalizer"
```

---

### Task 2: Add Compressor and Inspector nodes to `nodes_dataset.py`

**Files:**
- Modify: `nodes_dataset.py`

**Step 1: Append the Compressor and Inspector nodes**
...
```bash
git add nodes_dataset.py
git commit -m "feat: add Foley dataset Compressor and Inspector nodes"
```

---

### Task 3: Add Augmenter, HF Smoother, Saver, ItemExtractor to `nodes_dataset.py`

**Files:**
- Modify: `nodes_dataset.py`

**Step 1: Append the remaining dataset pipeline nodes**
...
```bash
git add nodes_dataset.py
git commit -m "feat: add Foley dataset Augmenter, HF Smoother, Saver, ItemExtractor"
```

---

### Task 4: Add SpectralMatcher (reference-based) to `nodes_dataset.py`

**Files:**
- Modify: `nodes_dataset.py`

**Step 1: Append the reference-based spectral matcher**

This is the most significant adaptation from SelVA. Instead of hardcoded VAE target means (which are specific to MMAudio's BigVGAN mel-spectrogram distribution), this version computes target statistics from a reference audio directory. The user provides a folder of audio files that sound good through DAC (e.g., output of FoleyVAERoundtrip), and the matcher EQs training clips toward that spectral profile.
...
```bash
git add nodes_dataset.py
git commit -m "feat: add Foley reference-based SpectralMatcher node"
```

---

### Task 5: Add single-audio nodes — HF Smoother, Harmonic Exciter, Output Normalizer

**Files:**
- Modify: `nodes_dataset.py`

**Step 1: Append the audio pre/post-processor nodes**

These operate on single `AUDIO` items (not datasets) — for use in inference pipelines or individual preview.
...
```bash
git add nodes_dataset.py
git commit -m "feat: add Foley HF Smoother, Harmonic Exciter, Output Normalizer"
```

---

### Task 6: Add node mappings and register in `__init__.py`

**Files:**
- Modify: `nodes_dataset.py`
- Modify: `__init__.py`

**Step 1: Append NODE_CLASS_MAPPINGS to nodes_dataset.py**
...
**Step 2: Update `__init__.py` to merge dataset node mappings**

The LoRA plan also modifies `__init__.py`. If the LoRA plan has already been executed, `__init__.py` will have LoRA imports. If not, it will still have the original. Either way, add the dataset import.

**If LoRA plan already applied** (check for `nodes_lora` import), add after the LoRA imports:
...
And update the merge line to include dataset mappings:

```python
NODE_CLASS_MAPPINGS = {**NODE_CLASS_MAPPINGS, **DATASET_NODE_CLASS_MAPPINGS}
NODE_DISPLAY_NAME_MAPPINGS = {**NODE_DISPLAY_NAME_MAPPINGS, **DATASET_NODE_DISPLAY_NAME_MAPPINGS}
```

**If LoRA plan NOT yet applied**, the full `__init__.py` should be:
...
**NOTE:** If both LoRA and dataset plans are applied, the final `__init__.py` merges all three:
...
```bash
git add nodes_dataset.py __init__.py
git commit -m "feat: register Foley dataset pipeline nodes in ComfyUI"
```
...
Run: `cd /media/p5/ComfyUI-HunyuanVideo-Foley && python -c "from nodes_dataset import NODE_CLASS_MAPPINGS; print(list(NODE_CLASS_MAPPINGS.keys()))"`

Expected: List of 13 node names

**Step 2: Verify dataset loader handles edge cases**

```python
python -c "
from nodes_dataset import FoleyDatasetLoader, FoleyDatasetResampler, FoleyDatasetItemExtractor
import tempfile, os
import torch, torchaudio

# Create temp folder with a test WAV
tmp = tempfile.mkdtemp()
wav = torch.randn(1, 48000)  # 1 second of noise at 48kHz
torchaudio.save(os.path.join(tmp, 'test.wav'), wav, 48000)

# Test loader
loader = FoleyDatasetLoader()
(ds,) = loader.load(tmp)
print(f'Loaded {len(ds)} clips, name={ds[0][\"name\"]}, sr={ds[0][\"sample_rate\"]}')

# Test resampler (should skip since already 48kHz)
resampler = FoleyDatasetResampler()
(ds2,) = resampler.resample(ds, 48000)
print(f'Resampled: {len(ds2)} clips')

# Test extractor
extractor = FoleyDatasetItemExtractor()
audio, name, total = extractor.extract(ds, 0)
print(f'Extracted: name={name}, total={total}, shape={audio[\"waveform\"].shape}')
...
**Step 3: Verify inspector detects issues**

```python
python -c "
from nodes_dataset import FoleyDatasetInspector
import torch

# Create a dataset with a clipping clip and a clean clip
dataset = [
    {'waveform': torch.ones(1, 1, 48000), 'sample_rate': 48000, 'name': 'clipped'},
    {'waveform': torch.randn(1, 1, 48000) * 0.1, 'sample_rate': 48000, 'name': 'clean'},
]

inspector = FoleyDatasetInspector()
(clean_ds, report) = inspector.inspect(dataset, True, 15.0, False, 0.5)
print(f'Clean: {len(clean_ds)}, Report: {report[:100]}...')
assert len(clean_ds) == 1, f'Expected 1 clean clip, got {len(clean_ds)}'
print('Inspector test PASSED')
"
```

Expected: `Inspector test PASSED`

**Step 4: Verify __init__.py merges correctly**

Run: `python -c "from __init__ import NODE_CLASS_MAPPINGS; ds_nodes = [k for k in NODE_CLASS_MAPPINGS if k.startswith('FoleyDataset') or k.startswith('FoleyHf') or k.startswith('FoleyHarmonic') or k.startswith('FoleyOutput')]; print(f'{len(ds_nodes)} dataset nodes registered: {ds_nodes}')"`

Expected: `13 dataset nodes registered: [...]`

**Step 5: Commit**

```bash
git add -A
git commit -m "test: verify Foley dataset pipeline integration"
```

---

## Parameter Comparison: Foley vs SelVA

| Parameter | SelVA Default | Foley Default | Reason |
|-----------|---------------|---------------|--------|
| **Resampler target_sr** | 44100 | 48000 | DAC codec operates at 48kHz |
| **HF Smoother cutoff** | 12000 Hz | 16000 Hz | DAC handles HF better than BigVGAN |
| **HF Smoother blend** | 0.7 | 0.5 | Less aggressive — DAC codec needs less HF attenuation |
| **Harmonic Exciter cutoff** | 3000 Hz | 4000 Hz | DAC preserves more natural HF |
| **Harmonic Exciter mix** | 0.15 | 0.10 | Less compensation needed |
| **SpectralMatcher** | Hardcoded VAE stats | Reference-based | DAC latent space differs from mel-spec |
| **SpectralMatcher n_mels** | 80/128 (mode) | 128 (always) | Single mode for 48kHz |
| **LUFS, Compressor, Inspector** | Same | Same | Audio-domain processing, codec-agnostic |

## Optional Dependencies

| Package | Required By | Install |
|---------|-------------|---------|
| `soxr` | Resampler | `pip install soxr` |
| `pyloudnorm` | LUFS Normalizer, Output Normalizer | `pip install pyloudnorm` |
| `pedalboard` | Compressor, Harmonic Exciter | `pip install pedalboard` |
| `soundfile` | Loader, Saver | `pip install soundfile` |
| `audiomentations` | Augmenter (pitch/stretch only) | `pip install audiomentations` |

All are imported at call time with clear error messages if missing. The core pipeline (Loader → Inspector → Saver) works with just `torchaudio` and `soundfile`.
