...
### Long-Run sfc20, Intensity Sampling & Content Separation (June 2026)
...
#### Denoise on MOANING attacks breath — strength sweep + hum notch (June 2026)
The gated denoise that won on blowjob (low SNR, non-breathy gags) **BACKFIRES on moaning.** Breath lives in the quiet gaps between moans — exactly where a spectral-gate denoiser operates — and **noisereduce cannot separate breath from noise** (proven on AlexisCrystal: a strength sweep gives a noise/breath removal ratio of **~1.0 at every strength** — it removes them equal). The source noise is **broadband** (+12 to +22 dB over a clean clip across the *entire* spectrum, sitting on top of the 300–4000 Hz breath band); the **only** separable component is a **60 Hz mains hum** (notchable at zero breath cost). So you can't tune denoise to spare the breath — you can only (a) use minimal broadband strength and (b) notch the hum. Strength sweep, breath cost measured on the noisy clips (gap 300–3000 Hz energy vs original, the RIGHT metric — total-HF gave a FALSE "preserved −0.2 dB" because it's dominated by the loud moans):

| denoise | noise floor removed | **breath removed** | output peak ckpt |
|---|---|---|---|
| strength 0.45 | −2.9 dB | **−2.1 dB** (clearly attacked) | ~5750 |
| strength 0.30 | −2.0 dB | **−1.2 dB** | ~5250–5750 |
| 0.20 + hum notch | −1.7 dB | **−0.8 dB** | ~6250 |
| **0.10 + hum notch** | −1.4 dB | **−0.5 dB** (barely touched) | **~5750** ← release |

Each halving of strength halves the breath loss; the **hum notch** lets the gentle broadband clean nearly as much as a stronger setting (it removes the tonal hum for free), so the ratio improves toward the gentle end. **0.10 + notch @5750 is the moaning release setting** — 5-video consensus, the clarity peak (HNR 6.03) coincides with peak breath (gap 14.5) and cleanest flatness, **no clarity-vs-breath tradeoff at the gentlest strength** (the harsher 0.2/0.3 versions trade breath for moan-purity; user ear: 0.1 moans "not disappointed", breath natural). Settled knobs: **stationary=True** (the noise IS stationary room-tone + hum; non-stationary tracks the transient breath AS noise — worse), **n_fft no effect** (1024/2048/4096 identical), **gate −47**. **VERDICT: denoise is CONTENT-SPECIFIC — blowjob (low SNR, non-breathy) → strong (0.6–0.7); MOANING (breathy) → MINIMAL (0.1) + hum notch, or skip if SNR already high (TT moaning 25 dB SNR = don't bother).** Method lesson: to judge breath, measure the **quiet-gap bands**, never total energy.
